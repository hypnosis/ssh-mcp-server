...
| Smithery | MCPB-бандл, не `smithery.yaml` | уже там; новую версию — `npm run build:mcpb` и `smithery mcp publish` под своей учётной записью |
| awesome-mcp-servers | PR в `README.md` | строка выше |

## Знаки в README

Верхний ряд — доверие, нижний — техника. Живыми оказались два новых:

| Знак | Откуда | Что показывает |
|---|---|---|
...
## Что Smithery считает и чего от неё не добиться

Счёт качества она собирает **из манифеста бандла**, а сервер не запускает. Метаданные
оттуда доезжают полностью, а раздел Capability Quality стоит на нуле, и поднять его нечем:
две спецификации требуют несовместимого.

- Схема манифеста MCPB (`mcpb-manifest-v0.3.schema.json`) разрешает у инструмента только
  `name` и `description`, при `additionalProperties: false`. Упаковщик отказывается паковать
  манифест с `inputSchema` — по строке на каждый инструмент.
- Их CLI берёт `manifest.tools` как есть, кладёт в `serverCard.tools` и отправляет; приёмник
  валидирует список схемой MCP Tool, где `inputSchema` обязателен. Бандл с правильным
  манифестом отвергается с `expected object, received undefined` — по строке на инструмент.

Выхода изнутри нет: любой бандл, который примет один валидатор, отвергнет другой. Дописать
поле в готовый архив мимо упаковщика тоже не годится — Smithery раздаёт этот же файл на
установку, и невалидный манифест сломает её тем, кто ставит оттуда. Поэтому списка
инструментов в манифесте нет намеренно, а нулевой раздел в их счёте — не наш недосмотр.
Ждать тут можно только их правки.

Верификация домена закрыта по другой причине. Она требует TXT-запись на хосте домашней
страницы, а домашняя страница — `github.com`, чужая зона. Верификация домена в GitHub Pages
здесь ни при чём: она доказывает права на **свой** домен, а не на `github.com`. Пункт
откроется только вместе с собственным доменом под проект.
