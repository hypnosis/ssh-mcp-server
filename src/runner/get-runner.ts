 ...
 * Выбор транспорта
 *
 * Пока идёт переход, транспортов два, и решает переменная окружения
 * `SSH_MCP_BACKEND`. Значение по умолчанию — прежний ssh2, поэтому обновление
 * пакета само по себе способ доставки команд не меняет. Оба транспорта дают
 * одинаковый результат: контракт проверяется одним набором тестов.
 *
 * Файл исчезает вместе с бэкендом ssh2 на последнем шаге спринта.
 */
...
const DEFAULT_BACKEND: RunnerBackend = 'ssh2';
