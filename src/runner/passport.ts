...
export const PASSPORT_PROBE_COMMAND =
  `sh -c 'printf "${MARKER} bash=%s sha256=%s coreutils=%s rsync=%s timeout=%s install=%s os=%s\\n" ` +
...
export function parsePassport(stdout: string): ServerPassport {
  ...
  for (const token of line.slice(line.indexOf(MARKER) + MARKER.length).trim().split(/\s+/)) {
