/**
 * Контракт CommandRunner — общий набор для всех бэкендов
 *
 * Транспорты разные (системный ssh, библиотека ssh2), а обещание одно:
 ...
 * Оба бэкенда прогоняют один и тот же набор, поэтому переключение флага
 * SSH_MCP_BACKEND не может незаметно изменить поведение инструментов.
 *
 * Файл не заканчивается на .test.ts намеренно: он не запускается сам,
 * его подключают тесты конкретных бэкендов.
 */
...
export interface RunnerHarness {
  /** Имя бэкенда для заголовка describe */
  name: string;
